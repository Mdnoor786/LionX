...
from userbot import BOTLOG, BOTLOG_CHATID, lionx
...
async def group_fban(event):
    ...
    if errors != []:
        ...
        for txt in errors:
            success_report += f"\n☞ __{txt}__"
    await eor(lionxevent, success_report)


@lionx.lion_cmd(
    pattern="unfban(?:\s|$)([\s\S]*)",
    command=("unfban", plugin_type),
    info={
...
async def quote_search(event):  # sourcery no-metrics
    ...
    if get_collection("fedids") is not None:
        ...
        feds = {}
    if fedgroup == "-all":
        lionxevent = await eor(event, "`Adding all your feds to database...`")
        fedidstoadd = []
        async with event.client.conversation("@MissRose_bot") as conv:
            try:
                ...
                if "can only" in response.text:
                    return await eod(lionxevent, f"__{response.text}__")
                ...
                await eod(lionxevent, f"**Error while fecthing myfeds:**\n__{e}__", 10)
...
async def fetch_fedinfo(event):
    ...
    lionxevent = await eor(event, "`Fetching info about given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            ...
            await eod(lionxevent, f"**Error while fecthing fedinfo:**\n__{e}__", 10)
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
...
async def fetch_fedinfo(event):
    ...
    lionxevent = await eor(event, "`Fetching admins list of given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
...
async def myfeds_fedinfo(event):
    ...
    lionxevent = await eor(event, "`Fetching list of feds...`")
...
async def fstat_rose(event):
    ...
    lionxevent = await eor(event, "`Fetching fedstat from given deatils...`")
    ...
    async with event.client.conversation(rose) as conv:
        try:
            ...
            await eod(lionxevent, f"**Error while fecthing fedstat:**\n__{e}__", 10)
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
