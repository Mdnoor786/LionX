...
from ..funcs.managers import eor, eod
...
async def grammer(event):
    ...
    await eod(event, "Checking for grammer.......")
    ...
    if wrongs:
        try:
            ...
            await eod(event, "An Error Occurred")
    if result != re_message.raw_text:
        return await eod(event, result)
    await eor(event, "__There is no grammer mistake in the replied text__", 7)
...
async def plagiarism(event):
    ...
    await eod(event, "Checking for plagiarism.......")
