...
from . import deEmojify, eod, eor, lionx, reply_id
...
async def tgscmd(message):
    ...
    if not reply:
        await eod(message, "`Reply to an animated sticker`", 3)
        return
...
async def parse(message):
    reply = await message.get_reply_message()
    if not reply:
        await eod(message, "<b>Reply to an Image!</b>", 3, parse_mode="html")
        return None, None
...
async def dotify(message, reply, pix, mode):
    await eor(message, "<b>Putting dots...</b>", parse_mode="html")
    count = 24
