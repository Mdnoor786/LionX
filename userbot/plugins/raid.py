...
async def add_ensns(event):
    ...
    if type:
        return
    lionxevent = await eor(event, "`Adding Raid to user...`")
    ...
    try:
        ...
        await eor(lionxevent, "Raid Has Been Started")


@lionx.lion_cmd(
    pattern="dreplyraid$",
    command=("dreplyraid", plugin_type),
    info={
