...
async def truth_dare_task(event):
    ...
    if taskmode == "task":
        lionxevent = await eor(event, "Getting a random task for you.....")
        taskmode = random.choice(["truth", "dare"])
    else:
        lionxevent = await eor(event, f"Getting a random {taskmode} task for you.....")
    ...
    try:
        ...
        if taskmode == "truth":
            await lionxevent.edit(f"**The truth task for you is**\n`{task}`")
        else:
            await lionxevent.edit(f"**The dare task for you is**\n`{task}`")
    except Exception as e:
        await eod(lionxevent, f"**Error while getting task**\n`{e}`", 7)


@lionx.lion_cmd(
    command=("truth", plugin_type),
    info={
...
async def igame(event):
    ...
    if input_str not in game_code:
        lionxevent = await eor(event, "`Give me a correct game code...`")
        await asyncio.sleep(1)
        await eod(
            lionxevent, f"**Available Game Codes & Names :-**\n\n{game_list}", time=60
        )
    else:
        await eor(
            event,
