...
async def _(event):
    ...
    try:
        ...
        if not total:
            await eod(event, f"**Usage:** `{chr}timer <seconds>`", 10)
            return
        t = int(total)
        pluto = await eor(event, "**Starting...**")
        while t >= 0:
            x = 3 if t > 300 else 1
            try:
...
async def app_search(event):
    ...
    if not app_name:
        await eod(event, f"**Usage:** `{chr}iapp <name>`", 10)
        return
...
async def _(event):
    ...
    if not args:
        await eor(event, f"**Usage:** `{chr}cid <number>`")
        return
    pluto = await eor(event, "**__Fetching details...__**")
    chat = "@RespawnRobot"
...
async def Gay(event):
    ...
    if not osho:
        return await eod(event, "`What to choose from`", 10)
    options = osho.split(",")
