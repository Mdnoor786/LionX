...
from . import lionx, swtmemes, useless
...
async def _(event):
    "fun animation"
    lionxevent = await eor(event, "`kiss`")
    animation_interval = 0.2
    animation_ttl = range(100)
    animation_chars = ["🤵       👰", "🤵     👰", "🤵  👰", "🤵💋👰"]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await lionxevent.edit(animation_chars[i % 4])


@lionx.lion_cmd(
    pattern="fuk$",
    command=("fuk", plugin_type),
    info={
...
async def _(event):
    "fun animation"
    lionxevent = await eor(event, "`fuking....`")
    animation_interval = 0.2
    animation_ttl = range(100)
...
async def _(event):
    "fun animation"
    lionxevent = await eor(event, "`sex`")
    animation_interval = 0.2
    animation_ttl = range(100)
    animation_chars = ["🤵       👰", "🤵     👰", "🤵  👰", "🤵👼👰"]
    for i in animation_ttl:
