...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**👨‍💻  Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of LionX are :")


@lionx.lion_cmd(
    pattern="dc$",
    command=("dc", plugin_type),
    info={
