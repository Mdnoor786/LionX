...
from ..plugins import ALIVE_NAME, USERID, LionX_grp, mention
...
@lionx.tgbot.on(CallbackQuery(data=re.compile(b"help_k_minu")))
@check_owner
...
@lionx.tgbot.on(CallbackQuery(data=re.compile(b"clise")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    buttons = [
...
@lionx.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@lionx.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@lionx.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
